...
public class CompleteTodoItemHandler : IRequestHandler<CompleteTodoItemCommand, TodoItemResponse>
{
    private readonly ILogger<CompleteTodoItemHandler> _logger;
    ...
    public CompleteTodoItemHandler(
        ...
        IValidator<CompleteTodoItemCommand> validator)
    {
    ...
    {
        _logger.LogInformation("Отметка задачи {Id} как завершённой.", request.Id);
        ...
        _logger.LogInformation("Задача с ID {Id} отмечена как завершённая.", request.Id);
